...
:fast
echo 正在启动 ODrive 上位机（真实硬件模式）...
echo 浏览器会在服务就绪后自动打开，通常约 2 秒。
echo 请勿关闭本窗口，关闭即退出程序。
echo.
"dist\odrive-gui\odrive-gui.exe"
goto end

:portable
echo 正在启动 ODrive 上位机 便携版（真实硬件模式）...
echo 单文件版每次需解包，约需 10-20 秒，请耐心等待。
echo 浏览器会在服务就绪后自动打开。请勿关闭本窗口。
echo.
"dist\odrive-gui-portable.exe"
goto end
...
".venv\Scripts\python.exe" backend\run.py --open
goto end
